...
export const HeadingBar = ({ table_layout, table_headings }) => {
    return (
        <div className={`heading-container grid min-[250px]:grid-cols-2 sm:grid-cols-5 rounded-lg gap-4 overflow-auto`}>
            {
...
export const ListItems = ({ HREmployeeState }) => {
    return (
        <>
            {HREmployeeState.data ? HREmployeeState.data.map((item) => <div className={`list-item-container grid min-[250px]:grid-cols-2 sm:grid-cols-5 py-1 gap-2 justify-center items-center border-b-2 border-blue-800`}>
                ...
                <div className="heading-content font-boldmin-[250px]:text-sm sm:text-xs lg:text-sm xl:text-lg p-2 rounded-lg text-center overflow-hidden text-ellipsis min-[250px]:hidden sm:block font-bold">
                    {item.department ? item.department : "Not Specified"}
                </div>
